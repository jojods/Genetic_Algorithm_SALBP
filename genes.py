...
Code = Union[List[int], List[float]]
...
class Individual(ABC):
    ...
    @abstractmethod
    def calc_number_violations(self) -> int:
        """
...
class StationGene(Individual):
    """
    Each StationGene is coded as a list of integers. The nth-element of the list corresponds to station asigned
    to the operation n.
    ...
    def __init__(self, operations, stations, code: Code, cross_type='SP', mut_type='random'):
    ...
    def calc_number_violations(self, graph):
        """
        Calculates the number of precedence violations
    ...
    def print_violations(self, graph):
        """
        Calculates the number of precedence violations
        ...
        for op in range(self.operations):
            for neighbor in graph[op]:
                if self.code[neighbor] < self.code[op]:
                    print(f"Atividade: {neighbor} - Estacao: {self.code[neighbor]}")
        return None
    ...
    def calc_fitness(self, gen, graph, times, k=1, scalling_factor=0):
        ...
        self.fitness = max(time_op) + (k * self.calc_number_violations(graph))
        self.gen = gen
    ...
    def crossover_SP(self, indv):
        ...
        ch1 = StationGene(self.operations, self.stations, code=copy.deepcopy(code_c1),
                            cross_type=self.crossover.__name__[-2:],
                            mut_type=self.mutate.__name__.split('_')[-1])

        ch2 = StationGene(self.operations, self.stations, code=copy.deepcopy(code_c2),
                            cross_type=self.crossover.__name__[-2:],
                            mut_type=self.mutate.__name__.split('_')[-1])
        return ch1, ch2
    ...
    def crossover_DP(self, indv):
        ...
        ch1 = StationGene(self.operations, self.stations, code=code_c1,
                            cross_type=self.crossover.__name__[-2:],
                            mut_type=self.mutate.__name__.split('_')[-1])

        ch2 = StationGene(self.operations, self.stations, code=code_c2,
                            cross_type=self.crossover.__name__[-2:],
                            mut_type=self.mutate.__name__.split('_')[-1])
        return ch1, ch2
